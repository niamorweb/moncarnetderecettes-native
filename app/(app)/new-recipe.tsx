...
import { recipesApi, categoriesApi, ExtractedRecipeData } from "@/utils/api";
import {
  Colors,
  FontSizes,
  FontWeights,
...
export default function NewRecipeScreen() {
  ...
  // Parse scan data if coming from scan
  const parseScanData = (): Partial<ExtractedRecipeData> | null => {
    if (params.source === "scan" && params.data) {
      try {
  ...
  const scanData = parseScanData();

  // Recipe data - pre-fill from scan if available
  const [name, setName] = useState(scanData?.nom || "");
  const [servings, setServings] = useState(
    scanData?.portions ? String(scanData.portions) : "4",
  );
  const [prepTime, setPrepTime] = useState(
    scanData?.temps_preparation ? String(scanData.temps_preparation) : "15",
  );
  const [cookTime, setCookTime] = useState(
    scanData?.temps_cuisson ? String(scanData.temps_cuisson) : "30",
  );
  const [selectedCategory, setSelectedCategory] = useState<string | null>(
    params.category || null,
  );
  const [ingredients, setIngredients] = useState<string[]>(
    scanData?.ingredients?.length ? scanData.ingredients : [""],
  );
  const [steps, setSteps] = useState<string[]>(
    scanData?.etapes?.length ? scanData.etapes : [""],
  );
  ...
  // Handle scan data when params change
  useEffect(() => {
    if (params.source === "scan") {
      setIsFromScan(true);

      // Parse and apply scan data
      if (params.data) {
        try {
          const data = JSON.parse(params.data) as Partial<ExtractedRecipeData>;
          if (data.nom) setName(data.nom);
          if (data.portions) setServings(String(data.portions));
          if (data.temps_preparation) setPrepTime(String(data.temps_preparation));
          if (data.temps_cuisson) setCookTime(String(data.temps_cuisson));
          if (data.ingredients?.length) setIngredients(data.ingredients);
          if (data.etapes?.length) setSteps(data.etapes);
        } catch (e) {
          console.error("Error parsing scan data:", e);
        }
      }
  ...
  }, [params.source, params.data, params.imageUri]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (imageUri) {
        // Create with image using FormData
        ...
        await recipesApi.createWithImage(formData);
      } else {
        ...
        {isFromScan && (
          <View style={styles.scanBanner}>
            <View style={styles.scanBannerIcon}>
              <Ionicons name="sparkles" size={20} color={Colors.success.main} />
            </View>
            <View style={styles.scanBannerContent}>
              <Text style={styles.scanBannerTitle}>
                Recette extraite par IA
              </Text>
